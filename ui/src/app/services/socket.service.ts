...
export class SocketService {
  ...
  createVehicleSocket(field:Field, x: number, y: number): VehicleSocket {
    return new VehicleSocket(field.id + '/' + UUID.UUID() + '/' + x + '/' + y, this.commandsMappingService);
  }

  getApplicationSocket(): ApplicationSocket {
    return new ApplicationSocket('supervisor/' + UUID.UUID(), this.commandsMappingService);
  }
